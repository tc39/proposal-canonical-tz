...
For example, `Europe/Kiev` and `Europe/Kyiv` will behave identically, except for methods like `toString` and `toJSON` that directly return the identifier.
...
Identifiers that refer to different countries continue treated as separate time zones in Temporal, because those countries' time zones may diverge in the future.
